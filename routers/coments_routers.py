...
from schemas.coment_schemaq import ComentShow
...
router = APIRouter(prefix="/coments", 
                   tags=["coment"],
...
@router.post("/")
async def new_post(content:str = Form(...),user: UserShow = Depends(current_user),id_post:int=Form(...)  ):
    try: 
